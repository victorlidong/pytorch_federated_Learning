...
def client_update(clientId,round,client_model, optimizer, train_loader, epoch,device):
    ...
    for e in range(epoch):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if(iteration%10==0):
                time_now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                log_str=time_now+",client id="+str(clientId)+",round="+str(round)+",epoch="+str(e+1)+",client update iteration="+str(iteration)+",loss="+str(loss.item())
                log_list.append(log_str)
                print(time_now," client id=",clientId,",round=",round,",epoch=",e+1,",client update iteration=",iteration,",loss=",loss.item())
...
def server_aggregate(global_model, client_models):
    ...
    for model in client_models:
        # add_noise(model, args.type, args.batch_size, args.clip_bound, args.pb, args.privacy_delta, args.sample_num,
        #       param_size, param_size_all, args.is_ratio_list,args.ratio_type)
        add_noise_test(model, args.type, args.batch_size, args.clip_bound, args.pb, args.privacy_delta, args.sample_num,
                  param_size, param_size_all, args.is_ratio_list, args.ratio_type,args.test_layer)
...
if __name__ ==  '__main__':
    ...
    #测试，只给指定层加噪声
    parser.add_argument('--test_layer',type=int,default=0)
    ...
    if args.islog:
        ...
        log_name=log_time+',Alexnet'+',type='+args.type+',pb='+str(args.pb) +'.batch_size='+str(args.batch_size) +'.log'
    ...
    global_model = AlexNet(10).to(device)
    print(global_model)
    client_models = [AlexNet(10).to(device).to(device) for _ in range(args.num_clients)]
    for model in client_models:
        model.load_state_dict(global_model.state_dict())
    ...
    for r in range(args.num_rounds):
        ...
        # client update
        loss = 0
        start_round_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        print("start_round_time",start_round_time)

        for i in range(args.num_clients):
            print("******client id ",i,"******")
            tmp_loss,tmp_log = client_update(i,r+1,client_models[i], opt[i], train_loader[client_idx[i]], args.epochs,device)
            loss+=tmp_loss

        end_round_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        print("end_round_time",end_round_time)
