...
if __name__ ==  '__main__':
    ...
    if args.islog:
        ...
        log_name=log_time+',Lenet'+',type='+args.type+',pb='+str(args.pb) +'.batch_size='+str(args.batch_size) +'.log'
    ...
    #训练数据
    train_dataset = datasets.MNIST(args.train_data, train=True, download=True,
    ...
    #测试数据
    test_dataset = datasets.MNIST(args.test_data, train=False,
                    transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize((0.1307,), (0.3081,))]))
    ...
    print(global_model)
    client_models = [LeNet().to(device).to(device) for _ in range(args.num_clients)]
    for model in client_models:
        model.load_state_dict(global_model.state_dict())
    ...
    for r in range(args.num_rounds):
        ...
        #     print("******client id ",i,"******")
        #     loss += client_update(i,r+1,client_models[i], opt[i], train_loader[client_idx[i]], args.epochs,device)
